...
```{r, results='hide'}
dcv <- readRDS("./data/prepared/Ages/zi_a3.rda")
str(dcv)
``` 
...
```{r, results='hide'}
dcrdv <- readRDS("./data/curated/crdv_rag.rda")
str(dcrdv)
``` 
...
idx <- is.na(dcrdvi$newConfPop_Vacc_Yes)
dcrdvi$newConfPop_Vacc_Yes[idx] <- 0
idx <- is.na(dcrdvi$newConfPop_Vacc_No)
dcrdvi$newConfPop_Vacc_No[idx] <- 0
...
dcrdvs.w.r <- dcrdv.w.r  %>%
  dplyr::mutate(sumAsymptomatic11 = sumRecovered*1,
                sumAsymptomatic12 = sumRecovered*2,
                sumAsymptomatic13 = sumRecovered*3,
                sumSusceptibles11= (Population - sumRecovered - sumAsymptomatic11) * (1-sumVaccinated_2/Population),
                sumSusceptibles12= (Population - sumRecovered - sumAsymptomatic12) * (1-sumVaccinated_2/Population),
                sumSusceptibles13= (Population - sumRecovered - sumAsymptomatic13) * (1-sumVaccinated_2/Population))
str(dcrdvs.w.r)
```
...
dp <- dcrdvs.w.r %>%
  ...
  dplyr::mutate(newConfirmed=newConfirmed/max(newConfirmed), 
                newRecovered=newRecovered/max(newRecovered), 
                newDeath=newDeath/max(newDeath),
...
ggplot(dp , aes(x=Date, y=Count, shape=Item, color=Item)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_x_datetime(date_breaks="months", date_labels="%Y-%m") +
  scale_y_continuous(limits=c(0,1), breaks=seq(0,2,by=.2), sec.axis=dup_axis()) +
...
dp <- dcrdv %>%
  ...
  dplyr::mutate(dplyr::across(starts_with("new"), ~ rollmean(.x, k=14, align="center", fill=0))) %>%
...
ggplot(data=dp %>% dplyr::filter(as.integer(AgeGroup)>5), aes(x=Date, y=Proportion, color=Item, fill=AgeGroup, shape=Gender)) +
  ...
  scale_y_continuous(limits=c(-.6,1), breaks=seq(-1,1,by=.2)) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  geom_line() + 
  geom_line(aes(y=0), size=0.25) +
...
### Estimate Susceptibles

```{r, results='hide'}

# Add Susceptibles to timeshifted data
dcrdvs.w <- dcrdv.w  %>%
  dplyr::mutate(sumAsymptomatic = sumRecovered*1,
                sumAsymptomatic11 = sumRecovered*1,
                sumAsymptomatic12 = sumRecovered*2,
                sumAsymptomatic13 = sumRecovered*3,
                sumSusceptibles  = (Population - sumRecovered - sumAsymptomatic)   * (1-sumVaccinated_2/Population),
                sumSusceptibles11= (Population - sumRecovered - sumAsymptomatic11) * (1-sumVaccinated_2/Population),
                sumSusceptibles12= (Population - sumRecovered - sumAsymptomatic12) * (1-sumVaccinated_2/Population),
                sumSusceptibles13= (Population - sumRecovered - sumAsymptomatic13) * (1-sumVaccinated_2/Population))
str(dcrdvs.w)
...
```{r}
dp <- dcrdvs.w %>%
  tidyr::gather(key="Item", value="Count", sumSusceptibles11, sumRecovered, sumVaccinated_1, sumVaccinated_2)
...
ggplot(data=dp %>% dplyr::filter(Date>as.POSIXct("2020-10-01")), aes(x=Date, y=Count, color=Item, shape=Item, linetype=Gender)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_x_datetime(date_breaks="2 months", date_labels="%b", sec.axis=dup_axis()) +
...
df <- dcrdvs.w.r %>%
  ...
  dplyr::mutate(futureDeath11=round(sumSusceptibles11 * meanDeathConf / asyptomaticCorrectionFactor)) %>%
  dplyr::mutate(futureDeath12=round(sumSusceptibles12 * meanDeathConf / asyptomaticCorrectionFactor)) %>%
  dplyr::mutate(futureDeath13=round(sumSusceptibles13 * meanDeathConf / asyptomaticCorrectionFactor)) %>%
  
  dplyr::mutate(across(starts_with("sum"), round)) %>%
  dplyr::mutate(meanDeathConf=round(meanDeathConf,4)) %>%
  #dplyr::mutate(allDeath=pastDeath+futureDeath) %>%
  dplyr::select(Date, Population, AgeGroup, Gender, sumSusceptibles11, sumSusceptibles13, sumRecovered, sumVaccinated_2, meanDeathConf, 
                popDeath,  pastDeath, futureDeath11, futureDeath13)

str(df)
```

Die nachfolgende Abschätzung der zu erwartenden Todesfälle trotz aktueller Durchimpfungsrate geht von folgenden Annahmen aus:

- Als Zeitpunkt der 'Voll-Immunisierung' ist der Tag der zweiten Impfung plus zwei Wochen definiert 
- Die Genesenen sind bei Wiederansteckung zu 100% gegen einen tödlichen Verlauf der geschützt
- Gleiches gilt für die asymptomatisch Erkrankten
- Alle nicht positiv Getesteten (d.h. alle, die die Krankheit nicht hatten oder nicht davon wussten) lassen sich in gleichem Ausmass impfen.
- Die Genesenen lassen sich nicht zwei mal impfen

Eine Reihe wichtiger Parameter ist (noch) nicht bekannt und kann (noch) nicht berücksichtigt werden, darunter

- Die altersabhängige Sterberate für die Delta Variante (wird aktuell aus der 3. Welle errechnet)
- Die Veränderung der Sterberate bei Wiederansteckung, insbesondere in den gefährdeten Altersgruppen

### Sterblichkeitsrate aus der 3. Welle
...
### Abschätzung der Todesfälle in der nicht immunisierten Bevökerung

Für eine erste Abschätzung wird davon ausgegangen, dass die Genesenen, die Asymptomatischen und die Geimpften zu 100% vor einem tödlichen Verlauf der Krankheit geschützt sind, konzentriert sich also auf die Sterbefälle in der nicht immunisierten Bevölkerung (Sterblichkeitsrate aus der 3. Welle).


### Abschätzung der Todesfälle in der nicht immunisierten Bevökerung

Die Anzahl der Immunisierten ist in Österreich mit Stand Anfang September etwa 60% der Bevölkerung. Daten zu Impfdurchbrüchen und Wiederansteckung liegen aber zum aktuellen Zeitpunkt nicht vor.
...
```
 ...
 - Tabelle
```{r}
df %>% dplyr::filter(Date==max(Date)-days(3)) %>% dplyr:::select(AgeGroup,  Gender, Population, sumRecovered, sumVaccinated_2, sumSusceptibles11, contains("Death"), -meanDeathConf) %>%
  dplyr::summarize(across(contains("Death"), sum))
```
