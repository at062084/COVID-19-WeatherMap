...
crdv.a3.at <- crdv %>%
  ...
  dplyr::mutate(sumVaccinated_X = Population - sumVaccinated_2)  %>%
  # assume as many asymptomatic as recovered -> asymptomatics are immunnized, but may have been vaccinated
  dplyr::mutate(sumImmunized = sumRecovered*asymFactor + sumVaccinated_2/Population*(Population-sumRecovered)) %>%
  dplyr::mutate(sumSusceptibles = Population - sumImmunized)
#str(crdv.a3.at)
...
- Der Schutz vor Ansteckung durch Genesung oder Impfung bestimmt sich aus dem Verhältnis der Inzidenz unter den Geimpften bzw. Genesenen gegenüber den NichtImmunisierten (weder Genesen noch 2xGeimpft). In den Formeln sind diese Gruppen mit 'I=Immunized (2xGeimpfte+Genesene)' und 'S=Susceptible' bezeichnet.
...
### Inzidenz nach Altersgruppe und ImpfStatus
...
ggplot(data=dp, aes(x=Date, y=Inzidenz, color=AgeGroup3, linetype=Vaccinated)) + 
  scale_y_continuous(sec.axis=dup_axis(), breaks=seq(0,100,by=20)) +
...
ggplot(data=dp, aes(x=Date, y=Inzidenz, color=AgeGroup3, linetype=Vaccinated)) + 
  ...
  scale_y_continuous(name="Inzidenz", trans="log2", limits=c(1/4,128),
...
dq <- crdv.a3.at %>% 
  ...
  dplyr::mutate(Immunized=sumImmunized/Population, Susceptible=sumSusceptibles/Population, BreakThroughs=Immunized*0.1,
                Genesen_x2=sumRecovered*2/Population, Vaccinated_2x=sumVaccinated_2/Population)
...
dp <- dq %>%
  ...
  dplyr::mutate(ImmunisierungsTyp=factor(ImmunisierungsTyp, 
                                     levels=c("Immunized", "Susceptible", "BreakThroughs", "Vaccinated_2x", "Genesen_x2"), 
                                     labels=c("Immunized", "Susceptible", "BreakThroughs", "Vaccinated_2x", "Genesen_x2")))
...
ggplot(data=dp, aes(x=Date, y=ImmunisierungsRate, color=AgeGroup3, linetype=ImmunisierungsTyp)) + 
  ...
  scale_color_manual(values=cbPalette) +
  geom_line() + 
  #geom_line(aes(y=sumImmunized/Population), linetype=1, size=0.5) + 
  #geom_line(aes(y=sumVaccinated_1/Population), linetype=3, size=.75) + 
  #geom_line(aes(y=sumVaccinated_2/Population), linetype=2, size=.75) + 
  #geom_point(data=crdv.a.5, aes(x=Date, y=0.5, color=AgeGroup), size=2) +
  facet_grid(.~AgeGroup3, labeller=label_both) +
  ggtitle("COVID-19 Österreich: Immunisierungsrate incl Genesene bei Impfeffektivität 90%")
``` 
...
```{r, results='hide'}
dp <- dq %>%
  tidyr::pivot_longer(cols=c("BreakThroughs","Susceptible"), 
                      names_to="ImmunisierungsTyp", values_to="ImmunisierungsRate") %>%
  dplyr::mutate(ImmunisierungsTyp=factor(ImmunisierungsTyp, 
                                     levels=c("Immunized", "Susceptible", "BreakThroughs", "Vaccinated_2x", "Genesen_x2"), 
                                     labels=c("Immunized", "Susceptible", "BreakThroughs", "Vaccinated_2x", "Genesen_x2")))
...
```{r, results='hide'}
dp <- dq %>%
  dplyr::mutate(BreaksThroughs_Susceptibles = BreakThroughs/Susceptible) 

dq <- yzi.a3 %>% dplyr::mutate(InzidenzVerhältnis=relConfPop)


ggplot(data=dp, aes(x=Date, y=BreaksThroughs_Susceptibles, color=AgeGroup3)) + 
  scale_x_datetime(date_breaks="months", date_labels="%b") +
  scale_y_continuous(limits=c(0,NA), breaks=seq(0,10,by=.1), sec.axis=dup_axis()) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  geom_line() + 
  geom_line(data=dq, aes(y=InzidenzVerhältnis)) +
  #geom_line(aes(y=sumImmunized/Population), linetype=1, size=0.5) + 
  #geom_line(aes(y=sumVaccinated_1/Population), linetype=3, size=.75) + 
  #geom_line(aes(y=sumVaccinated_2/Population), linetype=2, size=.75) + 
  #geom_point(data=crdv.a.5, aes(x=Date, y=0.5, color=AgeGroup), size=2) +
...
dp <- crdv.a3.at %>% 
  ...
  dplyr::select(Date, AgeGroup3, sumImmunized, sumSusceptibles)

for (e in seq(0.6,0.9,by=.1)) {
  dt <- dp %>%
    dplyr::mutate(newConfPop_Susc=1, newConfPop_Immu=newConfPop_Susc*(1-e)) %>%
    dplyr::mutate(newConfSusc=newConfPop_Susc*sumSusceptibles/100000, newConfImmu=newConfPop_Immu*sumImmunized/100000) %>%
    dplyr::mutate(Verhältnis_Immunisierte_NichtImmunisierte=newConfImmu/newConfSusc) %>%
    dplyr::mutate(ImmunisierungsGrad=paste0(e*100,"%"))
  if(e==0.6) {
    dq <- dt
  } else {
    dq <- rbind(dq,dt)
...
ggplot(data=dq, aes(x=Date, y=Verhältnis_Immunisierte_NichtImmunisierte, color=AgeGroup3, linetype=ImmunisierungsGrad)) + 
  scale_x_datetime(date_breaks="months", date_labels="%b") +
  scale_y_continuous(limits=c(0,NA), breaks=seq(0,10,by=.05), sec.axis=dup_axis()) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  geom_line() + 
  facet_grid(.~AgeGroup3, labeller=label_both) +
  ggtitle("COVID-19 Österreich: Verhältnis Immunisierte zu NichtImmunisierte")
...
ggplot(data=dp %>% dplyr::filter(Date>as.POSIXct("2021-07-01")), aes(x=Date, y=Inzidenz, color=AgeGroup, linetype=ConfSusPop)) + 
  scale_x_datetime(date_breaks="months", date_labels="%b") +
  scale_y_continuous(name="Inzidenz", trans="log2", limits=c(1/2,128),
                     breaks=2^(seq(-1,10,by=1)), labels=sprintf("%g",2^(seq(-1,10,by=1))), 
                     sec.axis=dup_axis(name="Inzidenz_Stufe", labels=seq(-1,10,by=1))) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
...
#  dplyr::mutate(Date=as.POSIXct.Date(Date)) %>%
  dplyr::arrange(Date, AgeGroup) %>%
  dplyr::group_by(Date, AgeGroup) %>%
...
dw <- data.frame(Date=sort(unique(crdv.a.w.w$Date)), WaveID="NoWave", stringsAsFactors=FALSE)
#str(dw)

waveStart=c(as.POSIXct("2020-10-20"), as.POSIXct("2021-03-10"), as.POSIXct("2021-08-16"), as.POSIXct("2021-09-13"))
waveName=c("Wave2","Wave3","Wave4","Flat4")
idx <- dw$Date>waveStart[1] & dw$Date<=(waveStart[1]+days(28))
dw$WaveID[idx] <- waveName[1]
idx <- dw$Date>waveStart[2] & dw$Date<=(waveStart[2]+days(28))
dw$WaveID[idx] <- waveName[2]
idx <- dw$Date>waveStart[3] & dw$Date<=(waveStart[3]+days(28))
dw$WaveID[idx] <- waveName[3]
idx <- dw$Date>waveStart[4] & dw$Date<=(waveStart[4]+days(28))
dw$WaveID[idx] <- waveName[4]
dw$WaveID <- factor(dw$WaveID, levels=waveName, labels=waveName)
# str(dw)
#table(dw$WaveID)

crdv.a.w.wx <- crdv.a.w.w %>%
...
crdv.a.w.wxy <- crdv.a.w.wx  %>%
  dplyr::filter(WaveID!="NoWave") %>%
  dplyr::group_by(AgeGroup, WaveID) %>%
  dplyr::summarize(.groups="drop", Population=first(Population),
                   newConfirmed = mean(newConfirmed), 
                   sumVaccinated_2=mean(sumVaccinated_2), sumImmunized=mean(sumImmunized), 
...
```{r, results='hide'}
ggplot(data=crdv.a.w.wxy, aes(x=AgeGroup, y=newConfPop, fill=WaveID, color=WaveID, shape=WaveID)) + 
...
```{r, results='hide'}
ggplot(data=crdv.a.w.wxz, aes(x=AgeGroup, y=propConfirmed, fill=WaveID, color=WaveID, shape=WaveID)) + 
  scale_y_continuous(limit=c(0,NA)) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  #geom_col(position="dodge") + 
...
```{r, results='hide'}
ggplot(data=crdv.a.w.wxz, aes(x=AgeGroup, y=propConfPop, fill=WaveID, color=WaveID, shape=WaveID)) + 
  scale_y_continuous(limit=c(0,NA)) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  #geom_col(position="dodge") + 
...
```{r, results='hide'}
ggplot(data=crdv.a.w.wxz, aes(x=AgeGroup, y=propConfSus, fill=WaveID, color=WaveID, shape=WaveID)) + 
  scale_y_continuous(limit=c(0,NA)) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  #geom_col(position="dodge") + 
...
```{r, results='hide'}
ggplot(data=crdv.a.w.wxz, aes(x=AgeGroup, y=propConfImm, fill=WaveID, color=WaveID, shape=WaveID)) + 
  scale_y_continuous(limit=c(0,NA)) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette) +
  #geom_col(position="dodge") + 
