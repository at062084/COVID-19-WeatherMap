...
```{r}
ds2 <- df %>% dplyr::filter(ID==2, ImpfStoff=="AstraZeneca", ImpfStatus=="ZweiteImpfung", Aktion=="AntiKörper")
lm2 <- lm(formula=log2(Wert)~Monat, data=ds2)
pred2 <- 2^predict(lm2, newdata=data.frame(Monat=Monat))
dp2 <- data.frame(Geschlecht="W", Monat=Monat, Wert=pred2, ImpfStoff="AstraZeneca")
```
...
ggplot(data=df %>% dplyr::filter(Aktion=="AntiKörper"), aes(x=Monat, y=Wert, shape=ImpfStoff, color=ImpfStatus, group=ImpfStatus)) +
  ...
  scale_y_continuous(limit=c(-50,2500), breaks=seq(0,2500,by=200), sec.axis=dup_axis(), expand=c(0.0,0.0)) +
  ...
  geom_line(data=dp2, aes(x=Monat, y=Wert), inherit.aes=FALSE, linetype=2) +
...
ggplot(data=df %>% dplyr::filter(Aktion=="AntiKörper"), aes(x=Monat, y=Wert, shape=ImpfStoff, color=ImpfStatus, group=ImpfStatus)) +
  ...
  scale_y_continuous(limit=c(10,2500), breaks=2^seq(0:12), sec.axis=dup_axis(), trans="log2") +
  geom_vline(data=df %>% dplyr::filter(Aktion=="Impfung"), aes(xintercept=Monat, color=ImpfStatus), size=1.5) +
  geom_line() +
  facet_grid(Geschlecht~.) +
  geom_line(data=dp1_2, aes(x=Monat, y=Wert), inherit.aes=FALSE, linetype=3) +
