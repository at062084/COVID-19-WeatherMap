...
RUN sudo chown -R shiny:shiny /srv/shiny-server \
    ...
    && sudo echo "14 14 * * * shiny cd /srv/shiny-server/COVID-19-WeatherMap && /usr/local/bin/Rscript ./cron.R" >> /etc/crontab \
    && sudo echo "23 23 * * * shiny cd /srv/shiny-server/COVID-19-WeatherMap && /usr/local/bin/Rscript ./cron.R" >> /etc/crontab
