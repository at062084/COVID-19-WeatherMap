...
APP_VER=`cat $APP_DIR/$DKR_PRJ/app.R | awk '{if(index($0,"COVID-19-WeatherMap-")){split($0,sp,"\"");id=sp[2];gsub("@","\\\@",id);print id}}'`
NEW_VER="COVID-19-WeatherMap-$GIT_VER IBM\@`date +%Y%m%d`"
